...
function DesignLab() {
  ...
  const vehicles = cars.map((car, index) => ({ name: car.name, tag: `${car.category} · projeto ${String(index + 1).padStart(2, '0')}`, power: car.spec.split(' • ')[0].replace(/[^0-9.]/g, ''), grade: ['S+', 'A+', 'S', 'A', 'S+', 'A'][index], progress: ['42', '28', '17', '12', '8', '5'][index], image: car.image }))
  useEffect(() => { window.localStorage.setItem('modlab-showcase-section', section); window.localStorage.setItem('modlab-showcase-selected', selected); window.localStorage.setItem('modlab-showcase-vehicle', String(vehicleIndex)); window.localStorage.setItem('modlab-showcase-steps', JSON.stringify(buildSteps)); window.localStorage.setItem('modlab-showcase-parts', JSON.stringify(savedParts)) }, [section, selected, vehicleIndex, buildSteps, savedParts])
  const options = {
