...
async function searchVehicles(query: string, year?: number): Promise<VehicleSearchResult[]> {
  ...
  const catalogMatches = cars.filter((car) => {
  ...
  const classicMatches = brazilianClassics.filter((car) => {
  ...
  if (!knownMakes.has(normalizeText(requestedMake))) return [...catalogMatches, ...classicMatches]
  ...
  return combined.filter((item, index) => combined.findIndex((candidate) => normalizeText(`${candidate.make} ${candidate.model}`) === normalizeText(`${item.make} ${item.model}`)) === index).slice(0, 12)
}
...
function SettingsPage() {
  ...
  return <section className="content-page settings-page">
    <div className="settings-profile"><span><UserRound size={28} /></span><div><strong>lightzins</strong><small>Conta Build Lab conectada ao GitHub</small><em><GitBranch size={12} /> github.com/lightzins</em></div><button className="secondary-action">Editar perfil</button></div>
    <div className="settings-section-title"><div><h2>Preferências gerais</h2><p>Personalize como o Build Lab funciona para você.</p></div></div>
