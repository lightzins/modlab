...
function DesignLab() {
  ...
  const searchGarage = async (event: FormEvent) => {
    ...
    try {
      ...
      const additions = results.map((result, index) => ({ name: `${result.make} ${result.model}`, tag: 'Busca pública · adicionar ao projeto', power: '—', grade: 'B', progress: '0', image: result.image }))
      setVehicles((currentVehicles) => [...currentVehicles, ...additions.filter((addition) => !currentVehicles.some((currentVehicle) => normalizeText(currentVehicle.name) === normalizeText(addition.name)))])
  ...
  if (screen === 'GARAGEM' && garageQuery.trim()) return <section className="showcase-site" aria-label="Resultados de busca da garagem">{renderShowcaseHeader()}<main className="showcase-search-page"><header><span className="eyebrow">BUSCA DE VEÍCULOS</span><h1>Resultados para<br />“{garageQuery}”.</h1><p>Escolha um modelo para abrir sua personalização.</p></header><form className="collection-search" onSubmit={searchGarage}><Search size={17} /><input value={garageQuery} onChange={(event) => setGarageQuery(event.target.value)} placeholder="Buscar modelo" aria-label="Buscar carro no catálogo" autoFocus /><button type="submit" disabled={searchingGarage}>{searchingGarage ? 'Buscando...' : 'Buscar online'}</button></form><div className="vertical-search-results">{visibleVehicles.length ? visibleVehicles.map((car) => { const index = vehicles.findIndex((item) => item.name === car.name); return <button key={car.name} onClick={() => selectVehicle(index)}>{car.image ? <img src={car.image} alt={car.name} /> : <span className="vertical-search-no-image"><ImageOff size={22} /> Imagem em validação</span>}<span><small>MODELO {String(index + 1).padStart(3, '0')}</small><strong>{car.name}</strong><em>{car.power === '—' ? 'Potência não informada' : `${car.power} cv · Classe ${car.grade}`}</em></span><ChevronRight size={18} /></button> }) : <div className="empty-search"><Search size={28} /><strong>Nenhum modelo local encontrado.</strong><p>Use “Buscar online” para consultar bases públicas e adicionar o carro ao catálogo.</p></div>}</div></main><footer className="showcase-footer"><button onClick={() => setGarageQuery('')}>Limpar busca e voltar à garagem</button><span><i /> {visibleVehicles.length} resultados</span></footer></section>
  ...
  if (screen === 'GARAGEM') return <section className="showcase-site" aria-label="Garagem Modlab"><header className="showcase-header"><a className="showcase-brand" href="/"><img src="/modlab-logo.png" alt="Modlab" /><strong>MODLAB</strong></a><nav>{['GARAGEM', 'BUILD', 'TUNING', 'MERCADO'].map((item) => <button onClick={() => setScreen(item)} className={item === screen ? 'active' : ''} key={item}>{item}</button>)}</nav><div className="showcase-profile"><i /><span>BETA LOCAL</span></div></header><div className="showcase-subnav"><span>GARAGEM / CATÁLOGO DE VEÍCULOS</span><span>{vehicles.length} MODELOS DISPONÍVEIS</span><button onClick={() => setVehicleIndex((index) => (index + 1) % vehicles.length)}><CarFront size={15} /> Trocar veículo</button></div><main className="showcase-collection showcase-collection--catalog"><header><span className="eyebrow">COLEÇÃO MODLAB</span><h1>Encontre o carro<br />da sua build.</h1><p>Catálogo inicial com {vehicles.length} modelos. Busque mais modelos em bases públicas.</p></header><form className="collection-search" onSubmit={searchGarage}><Search size={17} /><input value={garageQuery} onChange={(event) => setGarageQuery(event.target.value)} placeholder="Buscar modelo: Opala, Civic, Corolla, GT-R..." aria-label="Buscar carro no catálogo" /><button type="submit" disabled={searchingGarage}>{searchingGarage ? 'Buscando...' : 'Buscar e adicionar'}</button></form><div className="collection-grid">{visibleVehicles.map((car) => { const index = vehicles.findIndex((item) => item.name === car.name); return <button className={vehicleIndex === index ? 'selected' : ''} key={car.name} onClick={() => selectVehicle(index)}>{car.image ? <img src={car.image} alt={car.name} /> : <span className="collection-image-loading"><LoaderCircle className="spin" size={22} /> Buscando imagem do modelo</span>}<span><small>BUILD {String(index + 1).padStart(2, '0')}</small><strong>{car.name}</strong><em>{car.power === '—' ? 'Dados em validação' : `${car.power} cv · Classe ${car.grade}`}</em></span>{vehicleIndex === index && <i><Check size={15} /></i>}</button> })}</div><footer><span><b>{visibleVehicles.length}</b> modelos exibidos · role horizontalmente para explorar</span><button onClick={() => setScreen('TUNING')}>Abrir personalização <ChevronRight size={15} /></button></footer></main><footer className="showcase-footer"><span><b>BETA LOCAL FUNCIONAL</b> catálogo persistido neste navegador</span><span><i /> Imagens de referência associadas ao modelo</span></footer></section>
