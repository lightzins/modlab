...
function DesignLab() {
  ...
  const searchGarage = async (event: FormEvent) => {
    ...
    try {
      ...
      const additions = results.map((result) => ({ name: `${result.make} ${result.model}`, tag: 'Busca pública · configurar veículo', power: '—', progress: '0', image: result.image, imageYear: undefined }))
  ...
  if (screen === 'BUILD') return <section className="showcase-site" aria-label="Plano da build">
    {renderShowcaseHeader()}
    <main className="showcase-build showcase-build--workspace"><section><header><span className="eyebrow">BUILD / {vehicle.name}</span><h1>Rota da build</h1><p>{vehicle.version && vehicle.year ? `${vehicle.version} · ${vehicle.year}` : 'Defina versão e ano antes de validar a montagem.'}</p></header><div className="build-track">{[{ title: 'Veículo identificado', detail: vehicle.version && vehicle.year ? `${vehicle.version} · ${vehicle.year}` : 'Versão e ano pendentes' }, { title: 'Objetivo da build', detail: selected }, { title: 'Peças compatíveis', detail: savedParts.length ? `${savedParts.length} peça(s) separada(s)` : 'Monte sua lista no Mercado' }, { title: 'Instalação e acerto', detail: 'Liberado após confirmar componentes' }].map((step, index) => <button className={buildSteps[index] ? 'done' : ''} key={step.title} onClick={() => setBuildSteps((items) => items.map((value, itemIndex) => itemIndex === index ? !value : value))}><span>{buildSteps[index] ? <Check size={15} /> : String(index + 1).padStart(2, '0')}</span><div><strong>{step.title}</strong><small>{step.detail}</small></div><em>{buildSteps[index] ? 'Concluída' : 'Pendente'}</em></button>)}</div></section><aside><span className="eyebrow">PAINEL DA BUILD</span><div><small>VEÍCULO</small><strong>{vehicle.name}</strong><p>{vehicle.version && vehicle.year ? `${vehicle.version} · ${vehicle.year}` : 'Versão e ano pendentes'}</p></div><div><small>CONFIGURAÇÃO</small><strong>{selected}</strong></div><div><small>INVESTIMENTO PLANEJADO</small><strong>{savedParts.length ? currency.format(totalSaved) : 'Adicione peças'}</strong></div><button onClick={() => setScreen('TUNING')}><Wrench size={16} /> Voltar à oficina</button></aside></main><footer className="showcase-footer"><span><b>PLANO INTERATIVO</b> marque uma etapa somente quando ela estiver resolvida</span><span><i /> Progresso salvo neste dispositivo</span></footer>
  </section>
  if (screen === 'MERCADO') return <section className="showcase-site" aria-label="Mercado da build">
    {renderShowcaseHeader()}
    <main className="showcase-market showcase-market--workspace"><header><div><span className="eyebrow">CATÁLOGO DE COMPONENTES</span><h1>Peças para sua build.</h1><p>{vehicle.version && vehicle.year ? `Compatibilidade será validada para ${vehicle.name} ${vehicle.version} ${vehicle.year}.` : 'Informe versão e ano para receber compatibilidade específica.'}</p></div><span>{savedParts.length} na lista · {currency.format(totalSaved)}</span></header><div className="market-context"><CarFront size={18} /><div><small>VEÍCULO EM ANÁLISE</small><strong>{vehicle.name}</strong><span>{vehicle.version && vehicle.year ? `${vehicle.version} · ${vehicle.year}` : 'Versão e ano não informados'}</span></div><button onClick={() => setVehiclePickerOpen(true)}>Alterar veículo</button></div><div>{marketParts.map((part) => { const savedPart = savedParts.includes(part.name); return <article key={part.name}><span className="market-image"><Gauge size={26} /></span><div><small>{part.type}</small><h3>{part.name}</h3><p>{part.supplier} · compatibilidade a confirmar</p></div><strong>{part.price}</strong><button className={savedPart ? 'saved' : ''} onClick={() => setSavedParts((items) => savedPart ? items.filter((item) => item !== part.name) : [...items, part.name])}>{savedPart ? <Check size={15} /> : <Plus size={15} />}{savedPart ? 'Na build' : 'Adicionar'}</button></article> })}</div></main><footer className="showcase-footer"><span><b>LISTA DE COMPONENTES</b> preços e fornecedores são referências do protótipo</span><span><i /> Links de afiliado entram após integração</span></footer>
  </section>
  if (screen === 'GARAGEM') return <section className="showcase-site" aria-label="Garagem Modlab">
    ...
    <main className="showcase-collection showcase-collection--catalog">
      ...
      <div className="collection-grid">{visibleVehicles.map((car) => { const index = vehicles.findIndex((item) => item.name === car.name); return <button className={vehicleIndex === index ? 'selected' : ''} key={car.name} onClick={() => openVehicleSetup(index)}>{car.image ? <img src={car.image} alt={car.name} /> : <span className="collection-image-loading"><ImageOff size={22} /> Buscando imagem do modelo</span>}<span><small>MODELO {String(index + 1).padStart(3, '0')}</small><strong>{car.name}</strong><em>{car.version && car.year ? `${car.version} · ${car.year}` : 'Clique e informe versão + ano'}</em></span>{vehicleIndex === index && <i><Check size={15} /></i>}</button> })}</div>
